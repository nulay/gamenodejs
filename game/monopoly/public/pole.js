...
Gamer.prototype = {
    initialize:function(user,gameM, num){
        this.user = user;
        this.fishka = new Fishka(num,user.indexPosition,user.name,gameM.actSize);
        this.gameM = gameM;
        this.build();
    },
    build:function(){
        console.log('kkk:'+this.user.name+this.user.money);
        var t=Math.round(this.gameM.actSize.shCPl/4);
        var v=Math.floor(this.gameM.actSize.shCPl/4);
        this.vid=$('<div style="border-radius: '+this.gameM.actSize.fontsize+'px;font-size:'+this.gameM.actSize.fontsize+'pt;position:relative; display:inline-block;vertical-align:top;width: '+(v-20)+'px;height: '+(t-20)+'px; background-color: '+this.fishka.colorF[this.fishka.numF-1]+';background-image: url(/'+this.gameM.room.imageFolder+'/setka.png);margin: 5px;"><div style="color:'+this.fishka.colorText[this.fishka.numF-1]+';margin: 5px;">'+this.fishka.numF+' '+this.user.name+'</div></div>');
        this.bord=$('<img style="border-radius: '+this.gameM.actSize.fontsize+'px;position:absolute; top:0; left:0;" src="/'+this.gameM.room.imageFolder+'/ramka.png" width="'+(v-20)+'px" height="'+(t-20)+'px"/>');
        this.vid.append(this.bord);
        //if(this.user.money==null)alert('error build');
    ...
    updateVid:function(){
        //if(this.user.money==null)alert('error update');
...
Card.prototype = {
    ...
    buildCard:function(rasp){
        ...
        var img=$('<img src="/'+this.gameM.room.imageFolder+'/'+this.obj.image+'"/>').css('width',this.w+'px');
        ...
        if(this.obj.price){
            this.infoMoneyBG=$('<div style="z-index:-5;position:relative;background-image: url(/'+this.gameM.room.imageFolder+'/setka.png);">').css('height',Math.round(this.h/4)+'px').css('width',this.w+'px');
...
//            thisEl.vid.append(thisEl.selectVid);
    ...
    buyFilial:function(){
        ...
        this.poleStar.append('<img src="/'+this.gameM.room.imageFolder+'/'+imgS+'" width="'+razm+'px" height="auto"/>');
        this.recalculatePenalty();
    },
    putFilial:function(){
...
var gameMonopoly={
    ...
    setMigEl:function(el){
        ...
        this.migEl=el;
        if(t!=null) {
//            t.attr('src', '/' + this.room.imageFolder + '/ramka.png');
            t.fadeIn('slow');
        }
//        this.migEl.attr('src','/'+this.room.imageFolder+'/ramka.gif');
    },
    blink:function(){
        var thisEl=this;
    ...
    loadgamedata:function(){
        var thisEl=this;
        $.ajax({
            url: "/games/monopoly/loadgamedata",
            dataType : "json",
            data: {"roomName":thisEl.room.roomName},
            type: "GET"
        }).done(function(data) {
            if(data!=null) {
                ...
                if(this.keyLoad!=0){
                    ...
                    for(var i=0;i<data.listAction.length;i++) {
                        ...
                        // thisEl.loginfo(data.listAction[i].infoAction+" "+data.listAction[i].user);
                        //console.log("loadgamedata0 : " + thisEl.gamers[0].user.name + "/"+thisEl.gamers[1].user.name);
                        if (gamerA == null) {
                             alert('ERROR!!!!');
                             console.log("loadgamedata1 : " + data.listAction[i].user);
                             console.log("loadgamedata2 : " + JSON.stringify(data.listUser));
                             alert('diagnostic!!!!'+data.listUser.length);
                        }
                        if (gamerA != null) {
                            gamerA.user = thisEl.getUserByNameL(data.listAction[i].user, data.listUser);
                            //if(gamerA.user.money==null)alert(JSON.stringify(gamerA.user));
                            gamerA.updateVid();
                            thisEl.loginfo('user:' + gamerA.user.name + ' - ' + data.listAction[i].action+'('+data.listAction[i].infoAction+')');
                            ...
                            if (data.listAction[i].action == "CHANGE_USER") {
                                var gamer = thisEl.getUserByName(data.listAction[i].infoAction);
                                gamer.user = thisEl.getUserByNameL(data.listAction[i].infoAction, data.listUser);
                                //if(gamer.user.money==null)alert(JSON.stringify(gamer));                              
                                gamer.updateVid();
                            }
                            ...
                            if (data.listAction[i].action == "RECEIVE_INCOME") {
                                var gamer = thisEl.getUserByName(data.listAction[i].infoAction);
                                gamer.user = thisEl.getUserByNameL(data.listAction[i].infoAction);
                                //if(gamer.user.money==null)alert(JSON.stringify(gamer));
                                gamer.updateVid();
                            }
                            ...
                            if(data.listAction[i].action=="GO_PRISON"){
                                var gamer = thisEl.getUserByName(data.listAction[i].infoAction);                                
                                gamer.user = thisEl.getUserByName(data.listAction[i].infoAction);
                                thisEl.startFunctionGoPrison(gamer);
                            }
                            if(data.listAction[i].action=="EXCHANGE_OFFERS"){
                                if(data.listAction[i].infoAction.user==data.userRoom.name){
    ...
    getUserByName:function(name){
        ...
        for(var y=0;y<this.gamers.length;y++){
            console.log(JSON.stringify(this.gamers[y].user));
    ...
    loadPlace:function(){
        ...
        $.ajax({
            ...
            if(data!=null) {
                ...
                //thisEl.getStartGamers();
                thisEl.usersstart(thisEl.room.listUsers);
    ...
    getStartGamers:function(){
        var thisEl=this;
        $.ajax({
            url: "/games/monopoly/getStartGamers",
            dataType : "json",
            type: "GET"
        }).done(function(data) {
            thisEl.usersstart(data);
        });
    },
    usersstart:function(data){
       var thisEl=this;
       console.log("usersstart:"+thisEl.gamers.length+"/"+thisEl.room.maxCountUser+"/"+JSON.stringify(data));
       if(data!=null) {
    ...
    synhroGame:function(users){
        for(var i=0;i<users.length;i++){
            ...
            //console.log('synhroGame:'+JSON.stringify(gamer));
            if(gamer == null){
                var num = this.gamers.length;
                console.log('создаем пользователя'+users[i].name);
        ...
        for(var i=0;i<this.listCard.length;i++){
            if(this.listCard[i].obj.userOwner!=null){
                var g=this.getUserByName(this.listCard[i].obj.userOwner);
    ...
    buildPlace:function(listCardObj,size){
        var gC=Math.floor(listCardObj.length-4)/4;
        var topline=$('<div>');
        $('#poleGame').css('width',size.razmP+'px');
        $('#poleGame').css('font-size',size.fontsize+'pt');
        for(var i=0;i<=listCardObj.length/4;i++){
            var p='top';
            if(i==0 || i==Math.round((listCardObj.length)/4)){
                p='big';
            }
            topline.append(this.buildCard(listCardObj,i,p));
        }
        var centerLine=$('<div style="display: inline; clear: left;">');
        var leftCard=$('<div class="leftCard" style="width:'+size.shBc+'px;clear:left;float:left;">');
        for(var i=listCardObj.length-1;i>=(listCardObj.length-gC);i--){
            leftCard.append(this.buildCard(listCardObj,i,'left'));
        }
        centerLine.append(leftCard);
        centerLine.append('<div id="centerPl" class="centerPlace" style="position:relative;width:'+size.shCPl+'px;height:'+size.shCPl+'px;display: inline; float: left;"><img src="/'+this.room.imageFolder+'/'+this.room.imageCenter+'" width="'+size.shCPl+'px" height="'+size.shCPl+'px"/></div>');
        var rightCard=$('<div class="rightCard" style="width:'+size.shBc+'px;float:left;">');
        for(var i=listCardObj.length/4+1;i<=(listCardObj.length/4+gC);i++){
            rightCard.append(this.buildCard(listCardObj,i,'right'));
        }
        centerLine.append(rightCard);
        var bottomline=$('<div style="clear: left;">');
        for(var i=(listCardObj.length-gC-1);i>=(listCardObj.length-2*gC-2);i--){
            var p='bottom';
            if(i==(listCardObj.length-gC-1) || i==(listCardObj.length-2*gC-2)){
                p='big';
            }
            bottomline.append(this.buildCard(listCardObj,i,p));
        }
        this.poleGame.append($('<div id="place" style="float:left;">').append(topline).append(centerLine).append(bottomline));
    },
    ...
    buildSystemControl:function(){
        ...
        this.createBut('RETURN_CREDIT');
        //this.createBut('GAME_END');
        this.createBut('GAME_CLOSE');
        this.buttons['GAME_CLOSE'].hide().click(function(){
            document.location.href="/games/monopoly/actions/game_close";
        });

      //  this.buttons['RETURN_CREDIT'].click(function(){
      ...
      //  });
        ...
        this.imgName='/'+this.room.imageFolder+'/';
        ...
        this.panelReturnCredit=$('<div style="background:black;position:absolute;z-index:10;top:0;left:0;width:'+this.actSize.shCPl+'px;height:'+(t*2-2)+'px;"></div>').hide();
        this.panelReturnCredit.append('<div><label for="fname">Summ for return:</label><input type="text" id="sumreturncredit" name="sumreturncredit" value="0"/></div></div>');
        var panelButtonForReturnPanel=$('<div></div>');
        var buttonCancelReturnCredit=$('<button value="'+this.lang['CANCAL']+'">'+this.lang['CANCAL']+'</button>').click(function(){
            thisEl.panelReturnCredit().hide();
        });
        var buttonOkReturnCredit=$('<button value="Ok">Ok</button>').click(function(){
            var sum=parseInt($('#sumreturncredit').val());
            if(sum>0){
               thisEl.actionsUser('RETURN_CREDIT', );
               thisEl.panelReturnCredit().hide();
            }else{
               thisEl.loginfo('sum shold be more then 0');
            }
        });
        this.panelReturnCredit.append(panelButtonForReturnPanel);


        topPanel.append(this.buildChangePanel()).append(('<div style="background-color:black;opacity:0.85;width: '+this.actSize.shCPl+'px;height:'+(t*2)+'px;text-align:center;position:absolute;z-index:1;top:0;left:0;">')).append(this.butPanel).append(this.panelSelectFirm).append(this.panelReturnCredit).append(this.panelAuction).append(this.panelThrowCube);
    ...
    getPossibleFirm:function(action,userName){
        var thisEl=this;
        var urlD="/games/monopoly/actions/getPossibleFirm/"+action;
        if(userName){
            urlD="/games/monopoly/actions/getPossibleFirmCh/"+userName;
        }
        $.ajax({
            url: urlD,
            dataType : "json",
    ...
    actionsUser:function(actions, functObr) {
        ...
        console.log('actionsUser send ' + JSON.stringify(actions));
...
//                alert(datas);
        ...
        if (actions == 'RETURN_CREDIT'){
            datas = {'sum':$('#sumreturncredit').val()};           
        }
        this.hideAllBut();
        this.keyLoad=0;
        if(datas==null){datas={};}
        datas.action = actions.toLowerCase();
        datas.roomName = thisEl.room.roomName;
        console.log('send actions of User : '+JSON.stringify(datas));
        $.ajax({
            url: "/games/monopoly/actions",
            dataType : datat,
            data:datas,
        ...
        if(!ind){
            ...
            this.panelWin.append($('<div style="display: inline-block; vertical-align: middle; color: WHITE; padding: 60px;color:white; font-weight: bold;font-size: 20pt;"></div>')
                    .append('<div>'+this.lang['YOURS_WIN']+'</div>').append(this.imgWin).append($('<div>'+this.lang['GAME_CLOSE']+'</div>')
                            .click(function(){document.location.href="/games/monopoly/actions/game_close";})))
